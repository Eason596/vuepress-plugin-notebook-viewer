const nbv_constructor = (function(document, deps) {
    ...
    function render_ipynb(obj, target, settings) {
        ...
        t.className = dark ? 'nbviewer-root nbviewer-root-dark' : 'nbviewer-root nbviewer-root-light';
        t.setAttribute('style', [
            'max-width: 960px',
            'border: 1px solid ' + (dark ? '#444' : '#ccc'),
            'margin: 1em auto',
            'padding: 1.5em 1.5em 1.5em 7em',
            'background-color: ' + (dark ? '#1e1e1e' : 'white'),
            'color: ' + (dark ? '#d4d4d4' : '#213547'),
            'box-shadow: ' + (dark ? '0 0 10px rgba(0,0,0,0.45)' : '0 0 10px #ccc')
            ].join(';'));
        ...
        sheet.insertRule('table.dataframe tbody { border-top: 1px solid ' + (dark ? '#666' : '#4c4c4c') + '; }')
        sheet.insertRule('table.dataframe tbody th { text-align: right; }')
        sheet.insertRule('table.dataframe tbody tr:nth-child(odd) { background-color: ' + (dark ? '#2a2a2a' : '#f5f5f5') + '; }')
    }
    ...
    function handle_code(cell) {
        ...
        pre.setAttribute('style', [
            'background: ' + (dark ? '#2d2d2d' : '#f7f7f7'),
            'border: 1px solid ' + (dark ? '#555' : '#cfcfcf'),
            'padding: .4em',
            'margin-bottom: 0',
            'margin-top: 0',
            'border-radius: 2px',
...
if (typeof window !== "undefined") {
    window.nbv_constructor = nbv_constructor;
}
